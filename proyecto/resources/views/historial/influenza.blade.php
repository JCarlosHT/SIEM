...
<div class="container-fluid card p-4">
    <div class="row ">
        <div  class="col-md-12">
        	<h6>HISTORIAL DE SEMANAS EPIDEMIOLÓGICAS INFLUENZA </h6>
        	<table id="table" class="table align-items-center mb-0">
                    ...
                    <tbody>
                        @foreach($influenza as $id => $influ)
                        <tr class="border-bottom">
                          <td class="text-center">{{$influ->semana}}</td>
                          <td>{{$influ->anio}}</td>
                          <td class="td-short">{{$influ->usuario->nombrecompleto}}</td>
                          <td>{{$influ->updated_at->format('d-m-Y')}}</td>
                          ...
                          <td><a href="#" onclick="openiframe('Actualizar semana epidemiológica','{{route('act_inf',$influ->id)}}')" class="btn btn-link text-primary text-gradient p-2 mb-0"><i class="material-icons text-sm me-2">edit</i>Editar</a></td>
                        @endforeach
                        </tr>
                    </tbody>
                </table>
