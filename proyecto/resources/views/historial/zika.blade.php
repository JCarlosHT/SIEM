...
<div class="container-fluid card p-4">
    <div class="row ">
        <div  class="col-md-12">
        	<h6>HISTORIAL DE SEMANAS EPIDEMIOLÓGICAS ZIKA</h6>
        	<table id="table" class="table align-items-center mb-0">
                    ...
                    <tbody>
                        @foreach($zika as $id => $zik)
                        <tr class="border-bottom">
                          <td class="text-center">{{$zik->semana}}</td>
                          <td>{{$zik->anio}}</td>
                          <td class="td-short">{{$zik->usuario->nombrecompleto}}</td>
                          ...
                          <td><a href="#" onclick="openiframe('Actualizar semana epidemiológica','{{route('act_zik',$zik->id)}}')" class="btn btn-link text-primary text-gradient p-2 mb-0"><i class="material-icons text-sm me-2">edit</i>Editar</a></td>
                        @endforeach
                        </tr>
                    </tbody>
                </table>
