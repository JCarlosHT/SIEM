...
<div class="container-fluid card p-4">
    <div class="row ">
        <div  class="col-md-12">
        	<h6>HISTORIAL DE SEMANAS EPIDEMIOLÓGICAS HEPATITIS AGUDA</h6>
        	<table id="table" class="table align-items-center mb-0">
                    ...
                    <tbody>
                        @foreach($hepa as $id => $hep)
                        <tr class="border-bottom">
                          <td class="text-center">{{$hep->semana}}</td>
                          <td>{{$hep->anio}}</td>
                          <td class="td-short">{{$hep->usuario->nombrecompleto}}</td>
                          <td>{{$hep->updated_at->format('d-m-Y')}}</td>
                          ...
                          <td><a href="#" onclick="openiframe('Actualizar semana epidemiológica','{{route('act_hepa',$hep->id)}}')" class="btn btn-link text-primary text-gradient p-2 mb-0"><i class="material-icons text-sm me-2">edit</i>Editar</a></td>
                        @endforeach
                        </tr>
                    </tbody>
                </table>
