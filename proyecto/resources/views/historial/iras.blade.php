...
<div class="container-fluid card p-4">
    <div class="row ">
        <div  class="col-md-12">
        	<h6>HISTORIAL DE SEMANAS EPIDEMIOLÓGICAS IRAS </h6>
        	<table id="table" class="table align-items-center mb-0">
                    ...
                    <tbody>
                        @foreach($iras as $id => $ira)
                        <tr class="border-bottom">
                          <td class="text-center">{{$ira->semana}}</td>
                          <td>{{$ira->anio}}</td>
                          <td class="td-short">{{$ira->usuario->nombrecompleto}}</td>
                          <td>{{$ira->updated_at->format('d-m-Y')}}</td>
                          ...
                          <td><a href="#" onclick="openiframe('Actualizar semana epidemiológica','{{route('act_iras',$ira->id)}}')" class="btn btn-link text-primary text-gradient p-2 mb-0"><i class="material-icons text-sm me-2">edit</i>Editar</a></td>
                        @endforeach
                        </tr>
                    </tbody>
                </table>
